function thre_wplimin23_thval(subno,nfile,thp,lmtch)
% subno='01'; nfile='loc';
% thp: 15 20 25 30 35
% thp=20;
...
cd([direct '\wpli\'])
load (['sub' subno '_wpliminch23_' nfile '.mat'])

%%
wnetf=[]; bnetf=[]; fabth=[]; fpth=[];
lch=size(fwpli23,1); nconnect=lch*(lch-1)/2;

if lch < lmtch % over ch 15 or 20 or 25
    fprintf(['...low nconnect: stop thresholding....\n']);
else
    for ff=1:size(fwpli23,3)
        mattmp=fwpli23(:,:,ff); matsort=[];
        for i=1:lch   
    ...
    % save file
    cd([direct '\netwpli\'])
    s1=['save -v7.3 sub' subno '_thre_wpliminch23_thr' num2str(thp) '_' nfile '.mat lch nconnect wnetf bnetf fabth fpth']; eval(s1);
    fprintf(['...Thresholding done....\n']);
end
fprintf(['...Thresholding wplimin23 matrix Sub' subno ' ' nfile ' finished....\n']);
end