function coh_BAchall(cond,nfreq)

% sort all ch data BA*BA connectivity
% BAchlabel=subject, ch info
% BAchdata=coh value zBAchdata=fisher z(coh value)
...
for s=1:length(list_f) 
    ...
    cd([direct '\coh\']); load (['sub' subno '_cohmin_' cond '.mat'])
    ffmat=fcoh(:,:,nfreq); 
...
%% save file
cd(['E:\#ECoGconsciousness\Connect\all\coh\BAch'])
s1=['save -v7.3 BAchall_coh_' num2str(nfreq) fb{nfreq} '_' cond '.mat BAchlabel BAchdata zBAchdata lnBAchdata']; eval(s1);

fprintf(['...coh sort BA all ch data done....\n']);
