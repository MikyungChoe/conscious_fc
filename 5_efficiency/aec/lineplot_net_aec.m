...
load result_net_limitch20_aec.mat
...
for i=1:6 %delta to HG
    ...
    plot(x,y2,'-o','MarkerSize',3,'MarkerFaceColor','r','MarkerEdgeColor','r','linewidth',1.2)
    test=abs(allwspge); test(test>0.05)=NaN; test(test<0.05&test>0)=1*0.8;
    scatter(x,test(:,i), 10, "k", "*")
    set(gca,'FontSize',10,'FontWeight','bold','linewidth',1.2,'FontName','Arial');
    xlim([12 47]); ylim([0.3 0.9])       
end
set(gcf,'units','normalized','outerposition',[0 0 1 0.5]);
cd(['E:\#ECoGconsciousness\Connect\all\network\aec\fig'])
fname=['lineplot_net_lmt20_chthr_total']; saveas(gcf,fname, 'png');    
fname=['lineplot_net_lmt20_chthr_total']; saveas(gcf,fname, 'meta'); close(gcf);   
...
cd(['E:\#ECoGconsciousness\Connect\all\network\aec\fig'])
fname=['lineplot_net_lmt20_chthr1']; saveas(gcf,fname, 'png');    
fname=['lineplot_net_lmt20_chthr1']; saveas(gcf,fname, 'meta'); close(gcf);   
...
set(gcf,'units','normalized','outerposition',[0 0 0.7 1]);
cd(['E:\#ECoGconsciousness\Connect\all\network\aec\fig'])
fname=['lineplot_net_lmt20_chthr2']; saveas(gcf,fname, 'png');   
fname=['lineplot_net_lmt20_chthr2']; saveas(gcf,fname, 'meta'); close(gcf);   